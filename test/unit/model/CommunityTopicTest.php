...
$topic_comments = Doctrine::getTable('CommunityTopicComment')->createQuery()->orderBy('id')->execute();
$events = Doctrine::getTable('CommunityEvent')->createQuery()->orderBy('id')->execute();
$event_comments = Doctrine::getTable('CommunityEventComment')->createQuery()->orderBy('id')->execute();
...
function getAcl($object)
{
  ...
  switch(get_class($object))
  {
    ...
    case 'CommunityTopic':
      return opCommunityTopicAclBuilder::buildCollection($object->getCommunity(), $members);
    case 'CommunityTopicComment':
      return opCommunityTopicAclBuilder::buildCollection($object->getCommunityTopic()->getCommunity(), $members);
    case 'CommunityEvent':
      return opCommunityEventAclBuilder::buildCollection($object->getCommunity(), $members);
  }
}
...
$acl = getAcl($topics[0]);
$t->cmp_ok($acl->isAllowed(1, null, 'add'), '===', true, 'returns true for the community admin');
$t->cmp_ok($acl->isAllowed(2, null, 'add'), '===', true, 'returns true for the community topic author');
$t->cmp_ok($acl->isAllowed(3, null, 'add'), '===', true, 'returns true for a community member');
$t->cmp_ok($acl->isAllowed(4, null, 'add'), '===', false, 'returns false for a non-community member');
$acl = getAcl($topics[4]);
$t->cmp_ok($acl->isAllowed(4, null, 'add'), '===', false, 'returns false for the community topic author but a non-community member now');
...
// * topic_authority: admin_only
$t->diag('public_flag: auth_commu_member, topic_authority: admin_only');
$acl = getAcl($topic_comments[0]);
$t->cmp_ok($acl->isAllowed(1, null, 'delete'), '===', true, 'returns true for the community admin');
$t->cmp_ok($acl->isAllowed(2, null, 'delete'), '===', true, 'returns true for the community topic author');
$t->cmp_ok($acl->isAllowed(3, null, 'delete'), '===', false, 'returns false for a community member');
$t->cmp_ok($acl->isAllowed(4, null, 'delete'), '===', false, 'returns false for a non-community member');
$t->cmp_ok($acl->isAllowed(5, null, 'delete'), '===', true, 'returns true for the community topic comment author');

// * public_flag:     public
// * topic_authority: admin_only
$t->diag('public_flag: public, topic_authority: admin_only');
$acl = getAcl($topic_comments[1]);
$t->cmp_ok($acl->isAllowed(1, null, 'delete'), '===', true, 'returns true for the community admin');
$t->cmp_ok($acl->isAllowed(2, null, 'delete'), '===', true, 'returns true for the community topic author');
$t->cmp_ok($acl->isAllowed(3, null, 'delete'), '===', false, 'returns false for a community member');
$t->cmp_ok($acl->isAllowed(4, null, 'delete'), '===', false, 'returns false for a non-community member');
