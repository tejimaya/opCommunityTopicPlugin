 ...
 * @return string
 */
function op_community_topic_truncate($string, $width = 80, $etc = '', $rows = 1, $is_html = true)
{
  $rows = (int)$rows;
  if (!($rows > 0))
  {
  ...
  for ($i = 1; $i <= $rows; $i++)
  {
    ...
    $result[$i] = op_community_topic_truncate_callback($p_string, $width, $p_etc);
  }
  $string = implode("\n", $result);
...
function op_community_topic_truncate_callback($string, $width, $etc = '')
{
